...
const generateJSX = (nodes: JSONNode[]) => {
  return nodes.map((node, path) => {
    return generateComponent(node, path.toString());
  });
};

const generateComponent = (node: JSONNode | null, path: string = "") => {
    ...
    case "container":
      return (
          <Dustbin
            path={path}
            style={style}
            onClick={(e) => handleClick(e, { ...node, content: "" }, path)}
            key={path}
          >
            {Array.isArray(node.content) &&
              node.content.map((child, index) => (
                <React.Fragment key={index}>
                  {generateComponent(child, `${path}/${index}`)}
                </React.Fragment>
              ))}
          </Dustbin>
      );
